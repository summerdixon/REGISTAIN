...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        images = glob.glob('O:/Transplant_Testing/Blind_testing_Lung_MT/SP18*/Region*.mat')
        print(images)
        input_ = tf.placeholder(tf.float32, shape=[3248, 3248, 4])
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            tf.train.Saver().restore(sess, 'D:/Tairan/TransplantNetworkCode_Lung_HE/Models_cleaned_data_vlatest_nobleach/42000')
            ...
            for i in tqdm(range(len(images))):
               ...
               # path = 'outputs/{}'.format(images[i].split('\\')[-1].split('.')[0])
                if os.path.exists(os.path.dirname(images[i]).replace('SP','output_SP_Test').replace('O','Y').replace('MT','HE')) == False:
                    os.mkdir(os.path.dirname(images[i]).replace('SP','output_SP_Test').replace('O','Y').replace('MT','HE'))
                ...
                savepath = (images[i].replace('SP', 'output_SP_Test').replace('O','Y').replace('MT','HE'))
                ...
                image_af = scipy.io.loadmat(images[i])
                ...
                image_temp =  (x.copy())
                # for j in range(4):
                # image_temp[:, :, 0] = (image_temp[:, :, 0] == 0) * 1700 + (image_temp[:, :, 0]!=0) * image_temp[:, :, 0] # he:1700 mt:3500
                # image_temp[:, :, 1] = (image_temp[:, :, 1] == 0) * 700 + (image_temp[:, :, 1] != 0) * image_temp[:, :, 1]
                x[:, :] = (image_temp[:, :] - np.mean(image_temp[:, :])) / (np.std(image_temp[:, :])+1e-8)
                # x[:,:] = (image_temp[:,:] - np.mean(image_temp[:,:]))/(np.maximum(np.std(image_temp[:,:]), 4000)) \
                #           - (np.std(image_temp[:,:])<4000)*1 # he:2000 
                # # x[:, :, 0] = (np.squeeze(image_temp[:, :, 0]) - 7358.9) / 8732.8
                # # x[:, :, 1] = (np.squeeze(image_temp[:, :, 1]) - 1568.8) / 1580.3
                # x[:, :, 0] = (np.squeeze(image_temp[:, :, 0]) - 7175.9) / 6931.0
                # x[:, :, 1] = (np.squeeze(image_temp[:, :, 1]) - 1568.8) / 1580.3
               ...
               # y = np.load(images[i].replace('input', 'target'))[:1024,:1024,:]
                ...
                # y = np.array(label['target'], dtype = np.float32)
